...
function bootstrap_base_packages {
  ...
  if [ "10" -eq "$VERSION" ]; then
    ...
    zypper --non-interactive install -l gcc-c++ patch zlib zlib-devel libffi-devel sqlite3-devel libxml2-devel libxslt-devel libreadline5 readline-devel libopenssl-devel
  else
